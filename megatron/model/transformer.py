...
from megatron.model.fused_layer_norm import FusedRMSNorm
...
class ParallelGatedMLP(MegatronModule):
    ...
    def forward(self, hidden_states):
        # bias is None, since we do not use bias in LLaMA
        gate, bias = self.gate_proj(hidden_states)
        up, bias = self.up_proj(hidden_states)
        down, bias = self.down_proj(self.act_fn(gate) * up)
        return down, bias
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self, config,
        ...
        if self.layer_type in (LayerType.decoder,
            ...
            # Layernorm on the attention output.
            if config.normalization == 'RMSNorm':
                self.post_inter_attention_layernorm = FusedRMSNorm(
                    normalized_shape=config.hidden_size,
                    eps=config.layernorm_epsilon,
                    sequence_parallel=config.sequence_parallel,
                )
